...
- `/공지 종류:입장안내`, `/공지 종류:온보딩24시간`, `/공지 종류:온보딩48시간`, `/공지 종류:온보딩72시간`, `/공지 종류:역할전환안내`, `/공지 종류:온보딩연장안내`
  72시간 온보딩 흐름에 맞춰 운영진이 복사해 사용할 수 있는 공지 템플릿을 보여줍니다.
...
- `data/notices.json`
  `/공지` 명령어에서 사용하는 공지 템플릿입니다. 일정 안내, 봇 사용 안내, 온보딩 안내, 참여 리마인드, 문의 안내, 준비물, 결석 안내 문안을 관리합니다.
...
Slash Command 구조를 변경했을 때만 운영 서버에 `npm run deploy`로 명령어를 다시 등록합니다. 데이터만 수정했다면 보통 필요하지 않습니다. `/공지` 종류 선택지를 수정한 경우 실제 Discord 반영 전 `npm run deploy`가 필요합니다.

디스코드에서는 `/질문`, `/공지 종류:봇사용안내`, `/공지 종류:입장안내`, `/채널안내`를 직접 실행해 응답을 확인해 주세요.
