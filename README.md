...
프로젝트 리디파인 참여자 안내, FAQ, 온보딩, 리디파인 포인트, 상점, 교환, 체크인, 미션, 인증, 운영자 관리 기능을 제공하는 Discord 봇입니다.

MEE6는 포인트, EXP, 레벨업 운영에 사용하지 않습니다. 리디파인 봇이 실제 운영 안내, 참여 활동 기록, 리디파인 포인트, 교환, 운영자 처리 흐름을 담당합니다.
...
| `/포인트` | 참여자 | 본인 리디파인 포인트 확인 | 본인만 보이는 private 응답 중심 |
| `/상점` | 참여자 | 신청 가능한 리워드 확인 및 선택 | private 응답 중심, 코드를 외우지 않고 선택 메뉴로 진행 |
| `/교환` | 참여자 | 리디파인 포인트로 리워드 교환 신청 | 코드를 몰라도 선택 메뉴로 진행, 신청 코드 직접 입력 호환 |
...
참여자용 공개 사이트를 배포한 뒤 `PARTICIPANT_GUIDE_URL=https://...`을 설정하면 `/안내`와 `처음 왔다면 여기부터` 화면에 `참여자 가이드 사이트 열기`, `가이드 DM으로 받기` 버튼이 추가됩니다. DM 버튼은 참여자가 직접 누른 경우에만 짧은 시작 안내와 사이트 링크를 보내며, DM 수신이 막혀 있으면 현재 화면에서 사이트 링크를 다시 제공합니다. 사이트를 기준 문서로 두고, 그 밖의 개별 DM은 사용자가 먼저 말을 건 대화 연습이나 승인 결과처럼 개인 맥락이 필요한 안내에 사용합니다.
...
기본은 읽기 전용입니다. 교환·인증·포인트·미션/상점 쓰기를 활성화하려면 `ADMIN_WRITE_ENABLED=true`와 별도 `ADMIN_WRITE_TOKEN`을 함께 설정합니다. 같은 게이트에서 웹게임 flagged 판정과 기존 `/게임지급` 계약을 재사용한 주간 보상 미리보기·지급도 사용할 수 있습니다. 모든 쓰기 시도와 결과는 공통 운영 데이터 경로의 `admin-audit.local.json`에 남으며, 처리 버튼은 확인 단계를 거칩니다. 참여자 DM 처리는 포함하지 않습니다. 별도 `OPS_REMINDER_ENABLED`는 단일 인스턴스에서 운영 대기·마감 집계만 운영진 채널로 전송합니다.
