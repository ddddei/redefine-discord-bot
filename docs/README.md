...
| [operator-dashboard-guide.md](operator-dashboard-guide.md) | 운영자 허브 가이드 | `/운영현황`에서 운영 상태를 확인하고 다음 처리 명령어를 찾을 때 | 전체 요약, 오늘의 운영 큐, 첫날 점검, 교환/인증 대기, 포인트 로그, 미션/상점, 반응 승인 후속 확인, 도움 필요 신호, FAQ 후보, 백업 체크리스트 |
...
| [journey-point-system-plan.md](journey-point-system-plan.md) | 리디파인 포인트 시스템 기능개발문서 | 리디파인 포인트, 청년동 포인트 전환, 상점, 교환, 운영자 처리 흐름을 설계할 때 | 봇 역할 분리, 한글 명령어, 수동 전환 및 환불 흐름, 저장 방식, 정산 원칙 |
| [point-data-structure-plan.md](point-data-structure-plan.md) | 리디파인 포인트 시스템 데이터 구조 설계 문서 | 리디파인 포인트 구현 전 모델과 example JSON 구조를 확정할 때 | 데이터 모델, 상태값, 트랜잭션, 저장 방식, 오류 복구 기준 |
...
| [webgame-live-qa-checklist.md](webgame-live-qa-checklist.md) | 웹게임 실환경 QA 체크리스트 | 2026-07-13 비주얼 트랙 머지분과 /게임지급·admin 웹게임 지급을 실계정·실기기로 확인할 때 | Railway 선행 조건, 지급 멱등, 인앱 4게임 체크, 종합 판정 기준 |
...
- [webgame-rankings-ops.md](webgame-rankings-ops.md)
