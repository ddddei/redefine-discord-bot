...
프로젝트 리디파인 디스코드 봇의 여정 포인트 시스템 구현을 준비하기 위해 데이터 구조 설계 문서와 예시 JSON 파일을 작성한다.
...
- 여정 포인트 시스템 구현 전 데이터 구조를 정리하는 문서라고 설명한다.
...
- MEE6는 환대, 채팅 EXP, 레벨업 용도로만 사용한다.
- 리디파인 봇이 자체 여정 포인트를 관리한다.
- 여정 포인트는 한글 명령어 기반으로 운영한다.
- 청년동 포인트 지급 사이트와 직접 연동하지 않는다.
- /교환 신청 시 여정 포인트는 자동 차감된다.
...
users는 여정 포인트 잔액과 참여자 기본 식별 정보를 관리하는 모델이다.
...
- pending: 신청 완료, 여정 포인트 차감 완료, 청년동 포인트 지급 대기
- completed: 운영자가 청년동 포인트 또는 리워드 실제 지급 완료
- cancelled: 지급 불가 또는 오류로 신청 취소
- refunded: 취소 후 차감된 여정 포인트 환불 완료
...
- 여정 포인트 시스템의 데이터 모델, 상태값, 트랜잭션, 저장 방식, 오류 복구 기준을 정리한 설계 문서
