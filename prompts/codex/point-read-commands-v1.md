...
프로젝트 리디파인 디스코드 봇에 여정 포인트 조회형 명령어를 1차로 추가한다.
...
/포인트는 사용자가 자신의 여정 포인트 상태를 확인하는 명령어다.
...
- /포인트는 data/points.example.json을 읽되, 실제 Discord 사용자 ID가 example에 없으면 “아직 기록된 여정 포인트가 없어요” 또는 0P 안내를 표시한다.
...
- 여정 포인트는 참여를 돕는 선택형 요소라고 안내한다.
- 운영 데이터 저장 방식은 아직 확정 전이라는 점을 너무 길지 않게 안내한다.

응답 예시 방향:

- 제목: 내 여정 포인트
- 현재 보유 여정 포인트: 0P 또는 조회된 포인트
- 최근 거래가 있으면 최근 거래 3개 정도 표시
- 기록이 없으면 “아직 기록된 여정 포인트가 없어요.”
- “여정 포인트는 참여를 돕는 선택형 요소이며, 비교나 경쟁을 위한 점수가 아니에요.” 문구 포함
...
- 항목명, 설명, 필요 여정 포인트, 재고 또는 월 한도 정보를 간단히 표시한다.
- 응답은 가능하면 ephemeral로 한다.
