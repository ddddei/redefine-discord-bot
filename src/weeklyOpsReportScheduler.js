...
function reserveWeek(filePath, weekStartDateKst, now) {
  ...
  saveHistory(filePath, { ...history, records: [...history.records, record] });
  return record;
}
function finishWeek(filePath, weekStartDateKst, status, now, detail) {
  const history = readHistory(filePath);
...
function parseSchedule(env) {
  const weekday = Number(env.WEEKLY_OPS_REPORT_WEEKDAY || 1);
  const time = String(env.WEEKLY_OPS_REPORT_TIME_KST || '10:00');
  if (!Number.isInteger(weekday) || weekday < 1 || weekday > 7 || !/^([01]\d|2[0-3]):[0-5]\d$/.test(time)) return { weekday: 1, time: '10:00' };
  return { weekday, time };
}
...
async function sendWeeklyOpsReport(options = {}) {
  ...
  try { if (!reserveWeek(historyPath, weekStartDateKst, now)) return { ok: false, reason: 'ALREADY_RESERVED' }; } catch (error) { return { ok: false, reason: 'RESERVATION_FAILED' }; }
  ...
  if (!channelId) { finishWeek(historyPath, weekStartDateKst, 'skipped', now, 'MISSING_CHANNEL'); return { ok: false, reason: 'MISSING_CHANNEL' }; }
  let channel;
  try { channel = await fetchChannel(options.client, channelId); } catch (error) { finishWeek(historyPath, weekStartDateKst, 'failed', now, 'CHANNEL_FETCH_FAILED'); return { ok: false, reason: 'CHANNEL_FETCH_FAILED' }; }
  if (!channel || typeof channel.send !== 'function') { finishWeek(historyPath, weekStartDateKst, 'failed', now, 'CHANNEL_NOT_FOUND'); return { ok: false, reason: 'CHANNEL_NOT_FOUND' }; }
  let report;
  try { report = buildWeeklyOpsReportFromRepository(options.repository, { now, env, weekOffset }); } catch (error) { finishWeek(historyPath, weekStartDateKst, 'failed', now, 'DATA_READ_FAILED'); return { ok: false, reason: 'DATA_READ_FAILED' }; }
  try { await channel.send({ content: formatWeeklyOpsReportMessage(report, env.ADMIN_DASHBOARD_URL), allowedMentions: { parse: [] } }); }
  catch (error) { finishWeek(historyPath, weekStartDateKst, 'failed', now, 'SEND_FAILED'); return { ok: false, reason: 'SEND_FAILED' }; }
  finishWeek(historyPath, weekStartDateKst, 'sent', now);
