...
function createWebgamePayoutPreviewPayload(plan) {
  return {
    embeds: [
      createGuideEmbed('웹게임 주간 보상 지급 미리보기', buildPayoutPreviewLines(plan).join('\n'), {
        footer: OPERATOR_CHECK_FOOTER,
      }),
    ],
    components: [
      new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId(`${WEBGAME_PAYOUT_CONFIRM_PREFIX}${plan.weekKey}`)
          .setLabel('✅ 지급 승인')
          .setStyle(ButtonStyle.Primary),
        new ButtonBuilder()
          .setCustomId(WEBGAME_PAYOUT_CANCEL_ID)
          .setLabel('취소')
          .setStyle(ButtonStyle.Secondary)
      ),
    ],
...
async function handleInteractionCreate(interaction) {
  ...
  if (interaction.isButton && interaction.isButton()) {
    ...
    if (interaction.customId.startsWith(WEBGAME_PAYOUT_CONFIRM_PREFIX)) {
      await handleWebgamePayoutConfirmButton(interaction);
      return;
    }

    if (interaction.customId === WEBGAME_PAYOUT_CANCEL_ID) {
      await handleWebgamePayoutCancelButton(interaction);
      return;
    }
  ...
  if (interaction.commandName === '게임지급') {
    await handleWebgamePayoutCommand(interaction);
    return;
  }
